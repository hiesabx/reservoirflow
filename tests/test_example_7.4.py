...
class TestApp(unittest.TestCase):
    ...
    def test_pressures(self):
        pressures_desired = np.array([3959.4367684962185, 3878.3103054886546, 3797.1838424810912, 3716.057379473528])
        model = create_model()
    ...
    def test_well(self):
        ...
        np.testing.assert_almost_equal(model.wells[4]['pwf'], 3688.9926465271164, decimal=5)

    def test_rates(self):
        rates_desired = np.array([2304.0239999999912, 0.0, 0.0, 0.0, -600.0000000000036, -1704.0240000000003])
        model = create_model()
        model.solve(sparse=True, update=True, check_MB=True)
...
if __name__ == '__main__':
    def create_model():
        grid = grids.CartGrid(nx=4, ny=1, nz=1, dx=300, dy=350, dz=40, phi=0.27, kx=270, dtype='double')
        ...
        model.set_boundaries({0: {'pressure': 4000}, -1: {'gradient': -0.2}})
        return model
    unittest.main()
