...
def get_sampled_array_combined_distribution(
    input_array,
    num_samples,
):
    """
    ...
    if num_samples > num_elements:
        ...
        padded_indices = np.concatenate(
            (
                ...
                np.full(num_samples - num_elements, all_unique_indices_available[-1]),
            )
        )
    ...
    if num_geom_candidates > 0:
        ...
        geom_start_val = 1e-9
        geom_end_val = max(1e-9, split_index_value)  # Ensure positive end for geomspace
    ...
    unique_sorted_indices_pool = np.clip(unique_sorted_indices_pool, 0, max_index)
